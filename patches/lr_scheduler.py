...
Patch for Compatibility with PyTorch 2.7.x
------------------------------------------
This patch updates Dassl.Pytorch's custom LR schedulers to ensure compatibility with PyTorch ≥2.7.

Changes:
- Removed deprecated 'verbose' argument from _LRScheduler.__init__() calls and related method signatures.
- Based on PyTorch deprecation cycle:
  * In PyTorch 2.2 (Feb 2024), `verbose=True` began raising deprecation warnings.
  * In PyTorch 2.7.0 (March 5, 2025), `verbose` and `print_lr()` were fully removed via PR #147301.
