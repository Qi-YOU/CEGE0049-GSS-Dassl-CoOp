...
def load_checkpoint(fpath):
    ...
    In PyTorch 2.6, the default value of `weights_only` in `torch.load()` was changed from `False` to `True`.
    This means loading a full checkpoint (which includes objects like LR schedulers) will fail unless you:
      (1) Set `weights_only=False` explicitly (which we do here), or
      (2) Use `torch.serialization.add_safe_globals(...)` to manually allowlist custom objects (like schedulers).

    WARNING: Setting `weights_only=False` allows deserializing arbitrary Python objects,
    which could be dangerous if the checkpoint comes from an untrusted source.
    ...
    try:
        # Explicitly disable weights_only to avoid PyTorch 2.6 issues
        checkpoint = torch.load(fpath, map_location=map_location, weights_only=False)

    except UnicodeDecodeError:
        # Handle Python 2-saved pickle
